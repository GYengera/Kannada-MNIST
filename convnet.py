...
class network(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.avg_pool2d(torch.cat((x_5, x_3), dim=1), kernel_size=2, stride=2) #(128, 14, 14)

        x_5 = F.relu(self.conv2_5_bn(self.conv2_5(x))) #(128, 14, 14)
        x_3 = F.relu(self.conv2_3_bn(self.conv2_3(x))) #(128, 14, 14)
        x = F.avg_pool2d(torch.cat((x_5, x_3), dim=1), kernel_size=2, stride=2) #(256, 7, 7)

        x_5 = F.relu(self.conv3_5_bn(self.conv3_5(x))) #(256, 7, 7)
        x_3 = F.relu(self.conv3_3_bn(self.conv3_3(x))) #(256, 7, 7)
        x = F.avg_pool2d(torch.cat((x_5, x_3), dim=1), kernel_size=2, stride=1) #(512, 6, 6)
